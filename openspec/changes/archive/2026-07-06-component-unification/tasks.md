...
- [x] 7.1 Review worker outputs; merge only non-conflicting patches. (T0–T6
  worker output landed and reviewed in the main tree; no conflicting patches
  remained.)
- [x] 7.2 Run `cd src-tauri && cargo check` (host). (Green 2026-07-06.)
- [x] 7.3 Run compiler/debug_ast/component tests (host). (Component tests
  73/73 green; lock tests live in `compiler.rs` — `src/debug_ast.rs` is an
  orphaned scratch file with no test module. Unrelated pre-existing WIP
  failures in selector/destructuring/permissive-units tests are tracked
  outside this change.)
- [x] 7.4 Run G-KEY / G-EMIT / G-CORE / G-RENDER gates (host). (All green
  2026-07-06.)
- [x] 7.5 Update OpenSpec task checkboxes.

## Proof Gates

- [x] G-KEY Stable node keys byte-identical for all existing fixtures.
  (`fixture_lock_captures_stable_node_keys_for_all_parts` green.)
- [x] G-EMIT All fixtures roundtrip with original spellings.
  (`emit_spelling_lock_preserves_clause_heads_in_roundtrip` green.)
- [x] G-CORE No `ecky_core_ir` public struct changes.
  (`core_program_digest_lock_captures_structural_stability` green.)
- [x] G-RENDER Existing fixture artifact digests unchanged. (Lock digests
  recorded pre-change in T0 snapshots; recompile equality proven in 0.3.)
- [x] G-COMP A model using a nested, parameterized component renders through
  the native Direct OCCT path with correct per-instance verify tags.
  (Compile-level proof via parity + verify tests
  (`component_verify_clauses_travel_with_each_instantiation`,
  `nested_component_verify_clauses_travel_to_instantiating_part`,
  `component_verify_clauses_match_on_both_compile_paths`); native plan smoke
  `plans_nested_parameterized_component_for_direct_occt` green.)
