...
- [x] Run updated e2e specs — ran `app/qa/manual-code-apply/dialogue-mcp-thread` specs 2026-07-06: all declutter-updated assertions pass (49 green). 8 failures are unrelated in-flight drift: backend label copy rename ("mesh/EckyRust" → "Ecky Native") in qa.spec export-chooser/render tests, docs floating window, dialogue queue-mode annotations — tracked with the authoring-error-surface / native-occt work, not this change.
