...
- **FreeCAD Performance & Memory:** Complex boolean operations (like `cut` or `fuse`) on high-resolution lofted meshes (e.g. `z_steps > 50` and `angle_steps > 100`) combined with intense mathematical relief patterns (like Xeno Hive or Voronoi) will cause catastrophic Out-of-Memory crashes. ALWAYS strictly limit resolution parameters. Default live previews to very low resolution (e.g. 30x60 max) and explicitly warn users before generating high-res exports.
