<script lang="ts">
  ...
  function selectThread(thread: Thread) {
    ...
    if (activeThreadId === thread.id || loadingThreadId === thread.id) return;
  ...
  function threadHasUnreadAttention(threadId: string): boolean {
    return attentionThreadIds.includes(threadId);
  }
...
<div class="history-panel">
  ...
  <div class="history-list">
    {#each paginatedHistory as thread (thread.id)}
      ...
      <div 
        class="history-card status-{threadState.className} {activeThreadId === thread.id ? 'active' : ''} {loadingThreadId === thread.id ? 'loading' : ''}" 
        role="button"
        tabindex="0"
          ...
          {:else}
            {#if onFinalize && thread.versionCount > 0}
              <button
                ...
                title="Archive thread — move to inventory"
              >
                ARCHIVE
              </button>
            {/if}
...
<style>
  ...
  .status-badge.unread {
    border-color: color-mix(in srgb, var(--red) 85%, var(--bg-300));
    color: white;
    background: color-mix(in srgb, var(--red) 75%, #000 10%);
