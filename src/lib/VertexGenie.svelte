...
>
  ...
  >
    ...
    <g class="genie-corner-glyph">
      ...
      {#each glyph.cellPolygons as cell}
        <polygon class="genie-corner-cell" points={cell} />
      {/each}
      {#each glyph.dualEdges as edge}
        <line class="genie-corner-dual-edge" x1={edge.x1} y1={edge.y1} x2={edge.x2} y2={edge.y2} />
      {/each}
      ...
      {#each glyph.nodes as node, index}
        {#if index !== 6}
          <circle
            class="genie-corner-node"
            class:genie-corner-node--inner={index > 6}
            cx={node.x}
            cy={node.y}
            r={index > 6 ? 2.5 : 3.35}
          />
...
<style>
  ...
  .genie-corner-body {
    fill: color-mix(in srgb, var(--corner-body) 88%, var(--bg-100));
    stroke: color-mix(in srgb, var(--corner-edge) 82%, var(--corner-node));
    stroke-width: 3;
  ...
  .genie-corner-cell {
    fill: color-mix(in srgb, var(--corner-body) 74%, var(--bg-100));
    stroke: color-mix(in srgb, var(--corner-edge) 72%, transparent);
    stroke-width: 1.05;
    stroke-linejoin: miter;
    opacity: 0.82;
  }
  ...
  .genie-corner-dual-edge {
    stroke: color-mix(in srgb, var(--corner-node) 64%, transparent);
    stroke-width: 0.9;
    opacity: 0.5;
